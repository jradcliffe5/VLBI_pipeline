...
def fix_fields(vis):
	...
	_, idx, idx_rev = np.unique(field, return_index=True, return_inverse=True)
	idx_r = np.arange(0,mytb.nrows())[~idx]
	mytb.removerows(idx_r)
	mytb.flush()
	mytb.close()
