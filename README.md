...
* **Per field**: `params['qa']['per_field']` (default `true`) gives one plot per field. This is *not* shadeMS's own `--iter-field` - shadeMS always groups/indexes internally by `FIELD_ID` regardless of that flag (hardcoded into its `group_cols`), so `--iter-field` only changes whether output is split per field or combined at the end, not how much work it takes to get there. Instead this step calls shadeMS once per field (`--field <id>`, no `--iter-field`) for the same total work, but a stall or failure on one field doesn't lose the plots already written for the others - unlike a single `--iter-field` call, which loses everything built up so far.
...
* The default plot set mirrors eMCP's `make_4plots` (amp/phase vs time and amp/phase vs frequency) plus its `make_uvcov` (`params['qa']['plots']`, each `{"xaxis":...,"yaxis":...}`, with optional per-plot `corr`/`field`/`colour_by`/`per_field` overrides - the uv-coverage entry overrides `colour_by` to `none` so it shows plain sampling density). Add or remove entries freely - anything valid for shadeMS's `--xaxis`/`--yaxis` works (e.g. `chan`, `real`, `imag`). eMCP's elevation-vs-time plot has no equivalent here - elevation isn't an MS column, it needs an ephemeris calculation shadeMS doesn't do. Per column, plots sharing the same `corr`/`field`/`per_field`/`colour_by`-or-not are batched into a single shadeMS call (repeated `--xaxis`/`--yaxis`/`--colour-by`) rather than one call per plot, since shadeMS indexes the MS once per invocation and reuses that for every axis pair given - so batching avoids redundant re-indexing.
* `corr` defaults to whichever parallel-hand correlations are present (`RR,LL` or `XX,YY`), not cross-hand (`RL,LR`) - routine amp/phase QA doesn't need it, and on full-Stokes/polarimetric data it roughly doubles the bytes shadeMS has to read for no QA benefit. Override with an explicit `params['qa']['corr']` (or per-plot) for a polarization-calibration-specific check. `fields` defaults to all fields.
* Plots land in `params['qa']['outdir']` (default `plots/qa`, relative to `cwd`); each shadeMS call's own log goes to `<outdir>/call_logs/`.
* `params['qa']['row_chunk_size']` maps to shadeMS's `--row-chunk-size` (default is shadeMS's own, currently 5000 rows/task). On a large MS this can fan out into thousands of concurrent dask tasks/file reads; on a networked filesystem under metadata pressure that can stall on a single stuck request for a very long time rather than erroring - it looks exactly like a hang (all shadeMS threads idle except one blocked in the kernel, e.g. `wchan ceph_mdsc_wait_request` on CephFS - check with `ps -T -p <pid>`). If a run stalls like that, set this to something larger (e.g. `50000`-`100000`) to cut the task count down.
* `params['qa']['max_parallel']` (default `1` = sequential) runs up to that many shadeMS calls concurrently instead of one after another - since each field/column/selection-group is an independent call, this is a real wall-clock speedup rather than just avoiding redundant work. Keep it modest: each shadeMS call already parallelises its own reads internally via dask, and the stall `row_chunk_size` addresses came from too much concurrent I/O against the same networked filesystem, not too little.
