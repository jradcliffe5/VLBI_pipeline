...
if casa6 == True:
	...
	for i in ['amp','phase']:
		for j in ['freq','time']:
			plotcaltable(caltable='%s/caltables/%s.bpass'%(cwd,p_c),yaxis='%s'%i,xaxis='%s'%j,plotflag=True,msinfo=msinfo,figfile='%s/plots/caltables/%s-bpass_%s_vs_%s.pdf'%(cwd,p_c,i,j))
